...
export class AZNavigation extends React.Component<
  IAZNavigationProps,
  IAZNavigationState
> {
  ...
  public render(): React.ReactElement<IAZNavigationProps> {
    //Create # symbol array
    let numberArray = () => {
      return Array.apply(null, { length: 1 }).map((num: any, index: number) =>
        String.fromCharCode(35 + index)
      );
    };
    ...
    let arrayAtoZ = () => {
      return Array.apply(null, { length: 26 }).map((num: any, index: number) =>
    ...
    return (
      <Pivot
        styles={pivotStyles}
        onLinkClick={this._handleClick}
        selectedKey={this.props.selectedLetter}
      >
